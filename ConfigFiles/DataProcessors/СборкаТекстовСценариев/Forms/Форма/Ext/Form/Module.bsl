...
Процедура СделатьСборкуСценариев() Экспорт
	...
	Объект.ЕстьОтборПоНастройкам =
		(Объект.ОтборПоНастройкамПроцессов.Количество() > 0
		 ИЛИ Объект.ОтборПоНастройкамСценариев.Количество() > 0 
		 ИЛИ Объект.ОтборПоUIDНастроекСценариев.Количество() > 0);
	
	Объект.ЕстьОтборПоТестам =
		(Объект.ОтборПоПроцессам.Количество() > 0
		 ИЛИ Объект.ОтборПоСценариям.Количество() > 0);
	...
	Если Объект.Ошибки.Количество() > 0 Тогда
		...
		ЛогВыполнения = СтрШаблон(НСтр("ru = 'При выполнении возникли ошибки: %1'"), ЛогВыполнения);
				...
				Если Элем.ДанныеФайлаYaml["ТипФайла"] = "НастройкаТеста" Тогда
					...
					Если Элем.Свойство("ДанныеСценариевКоторыеДобавляютсяКОсновному") Тогда
						Для Каждого ДанныеДополнительногоСценария Из Элем.ДанныеСценариевКоторыеДобавляютсяКОсновному Цикл
							Если ДанныеДополнительногоСценария.UIDСценария <> UIDСценария Тогда
...
Процедура ЗаполнитьПараметрыОбработки(СтруктураПараметров)
	...
	Если СтруктураПараметров.Свойство("scenariofilterhaspriority") Тогда
		Объект.ОтборСценариевИмеетПриоритет = СтрокуВБулево(СтруктураПараметров.scenariofilterhaspriority);
	КонецЕсли;
