...
Функция RegisterErrors(CommonData, ErrorsTable)
	...
	Если ТаблицаОшибок.Колонки.Найти("УИДОшибки") = Неопределено Тогда
		ТаблицаОшибок.Колонки.Добавить("УИДОшибки");
	КонецЕсли;
	
	Если ТаблицаОшибок.Колонки.Найти("Уточнение") = Неопределено Тогда
		
		МассивОшибокДляЗаписи = Новый Массив;
		Для Каждого ОшибкаДляЗаписи Из ТаблицаОшибок Цикл
			НайденнаяОшибка = ИнтеграцияСАПК.СсылкаПоИдентификатору(Справочники.Ошибки, ОшибкаДляЗаписи.УИДОшибки);
			Если НайденнаяОшибка.Пустая() ИЛИ НайденнаяОшибка.ПометкаУдаления Тогда
				МассивОшибокДляЗаписи.Добавить(ОшибкаДляЗаписи);
			КонецЕсли;
		КонецЦикла;
		
		Если МассивОшибокДляЗаписи.Количество() = 0 Тогда
			Возврат ИнтеграцияСАПК.ВернутьОшибкуСервиса(НСтр("ru='Ошибки не зарегистрированы'"), НСтр("ru='Найдены дубли ошибок'"));
		КонецЕсли;
		...
		Для Каждого ТекСтр Из МассивОшибокДляЗаписи Цикл
