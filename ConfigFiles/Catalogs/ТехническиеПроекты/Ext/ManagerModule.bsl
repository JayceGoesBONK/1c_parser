...
Функция ТекстЗапросаДляФормированияОписания()
	...
	|	ТехническиеПроекты.ПубликуемоеНаименование КАК ПубликуемоеНаименование,
	...
	|	ТехническиеПроекты.Ветка.ДатаПомещения КАК ДатаПомещенияИзмененийВВеткуВерсии,
...
Процедура СформироватьОписаниеОбъекта(Выборка, ТаблицаФайлов, Документ)
	...
	Иначе
		...
		ОписаниеОбъектов.ВывестиТекстПоАбзацам(НСтр("ru='Публикуемое наименование: '") + Выборка.ПубликуемоеНаименование, 1, Документ);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Выборка.ДатаПомещенияИзмененийВВеткуВерсии) Тогда
		
		ИнформацияОПомещенииВХранилище = НСтр("ru='Изменения помещены в ветку версии'");
		
		ИнформацияОПомещенииВХранилище = ИнформацияОПомещенииВХранилище + " "
				+ Формат(Выборка.ДатаПомещенияИзмененийВВеткуВерсии, "ДЛФ=DT");
