	...
	Если РазрабатываемыйОбъект <> Неопределено Тогда
		...
		ИначеЕсли ТипРазрабатываемогоОбъекта = Тип("СправочникСсылка.ТехническиеПроекты") Тогда
			...
			Если РазрабатываемыйОбъект.Статус = Перечисления.СтатусыТехническихПроектов.Выполнен Тогда
				...
				Ветка.ДатаПомещения = РазрабатываемыйОбъект.ДатаПомещенияИзмененийВВеткуВерсии;
...
Функция ИмяВеткиРепозиторияПоРазрабатываемомуОбъекту(РазрабатываемыйОбъект, Ответственный=Неопределено, 
	...
	ИначеЕсли ТипРазрабатываемогоОбъекта = Тип("СправочникСсылка.Ошибки") 
		...
		МассивСтрок.Добавить("/");
		ДатаСтрокой = Формат(ДатаСоздания, "ДФ=yyyyMMdd");
...
Функция ТехническиеПроектыИспользующиеВетку(Ветка) Экспорт
	...
	|	ПРЕДСТАВЛЕНИЕ(ТехническиеПроекты.Версия) КАК ВерсияПредставление
	|ИЗ
	...
	|
	|ВЫБРАТЬ
	|	ТехническиеПроектыВерсии.Ссылка,
	|	ТехническиеПроектыВерсии.Ссылка.Код,
	|	ТехническиеПроектыВерсии.Ссылка.Версия,
	|	ПРЕДСТАВЛЕНИЕ(ТехническиеПроектыВерсии.Ссылка.Версия)
	|ИЗ
	|	Справочник.ТехническиеПроекты.Версии КАК ТехническиеПроектыВерсии
	|ГДЕ
	|	ТехническиеПроектыВерсии.ВеткаРазработки = &Ветка
	|	И НЕ ТехническиеПроектыВерсии.Ссылка.ПометкаУдаления";	
...
Функция УбратьЗапрещенныеСимволыИзИмениВетки(Знач Стр)
	...
	Стр = СтрЗаменитьПоРегулярномуВыражению(Стр, "[^A-Za-z0-9_\-\.\/]+|\.$", "");
