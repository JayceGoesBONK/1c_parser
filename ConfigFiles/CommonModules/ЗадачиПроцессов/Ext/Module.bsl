...
Функция ПредставлениеТекущегоЭтапаПроцесса(Предмет) Экспорт
	...
	Если ЗначениеЗаполнено(Предмет)
	    И ВидПланаЗанятостиНастройки <> Неопределено
...
Процедура СформироватьПодменюПоПлануЗанятости(Форма, ПараметрыНастроек) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьПланирование")
		Или Не ПравоДоступа("Просмотр", Метаданные.Справочники.ВидыПланов) Тогда
...
Функция РеквизитыСогласованияСозданы(Форма)
	...
	Возврат ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "СогласующиеПоПредмету");
...
Процедура ВывестиИнформациюСогласующихПоПредмету(Форма)
	...
	Если Форма.СогласующиеПоПредмету.Количество() = 1
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "СогласующийРесурс") Тогда
		
		Форма.СогласующийРесурс = Форма.СогласующиеПоПредмету[0].Согласующий;
		
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПредставлениеСогласующиеПоПредмету") Тогда
...
Функция РезультатИзмененияРодителяУМассиваЗадач(МассивЗадачКИзменениюРодителя, НоваяЗадачРодитель) Экспорт
	...
	|ГДЕ
	|ИерархияЗадачПроцесса.Родитель В (&МассивЗадач)
	|И НЕ ИерархияЗадачПроцесса.ЗадачаПроцесса В (&МассивЗадач)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////2
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	ТаблицаДочернихЗадач                        = РезультатЗапроса[1].Выгрузить();
	ВыборкаЗадачиКИзмениюРодителя               = РезультатЗапроса[2].Выбрать();
	ВыборкаРеквизитыНовогоРодителя              = РезультатЗапроса[3].Выбрать();
	ВыборкаНовыйРодительЯвляетсяДочернейЗадачей = РезультатЗапроса[4].Выбрать();
	...
	ПредметПереносимойЗадачи              = ВыборкаЗадачиКИзмениюРодителя.Предмет;
	...
	Пока ВыборкаЗадачиКИзмениюРодителя.Следующий() Цикл
		...
		Если ПредметПереносимойЗадачи <> ПредметНовойЗадачиРодителя Тогда
			ОтразитьИзменениеПредметаПодчиненныхЗадачПереносимой(ВыборкаЗадачиКИзмениюРодителя.Ссылка, ТаблицаДочернихЗадач, ПредметНовойЗадачиРодителя);
		КонецЕсли;
...
Процедура ОтразитьИзменениеПредметаПодчиненныхЗадачПереносимой(РодительскаяЗадача, ТаблицаДочернихЗадач, НовыйПредмет)

	ПараметрыПоиска = Новый Структура;
	ПараметрыПоиска.Вставить("Родитель", РодительскаяЗадача);
	
	НайденныеСтроки = ТаблицаДочернихЗадач.НайтиСтроки(ПараметрыПоиска);
	
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		
		ЗадачаОбъект = НайденнаяСтрока.ЗадачаПроцесса.ПолучитьОбъект();
		ЗадачаОбъект.Предмет = НовыйПредмет;
		ЗадачаОбъект.ДополнительныеСвойства.Вставить("НеВыполнятьРасчет", Истина);
		ЗадачаОбъект.Записать();
