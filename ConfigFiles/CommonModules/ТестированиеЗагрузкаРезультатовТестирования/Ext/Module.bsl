...
Процедура ОбработатьДанныеЗапускаТестированияGitLab(Pipe, ДанныеПроекта, Соединение, ЗагруженныеВетки=Неопределено,
	...
	Для Каждого ЗаданиеGitLab Из ЗаданияGitLab Цикл
		...
		НРегИмя = НРег(ЗаданиеGitLab.name);
		
		Если Лев(НРегИмя, 4) = "bdd "
			ИЛИ Лев(НРегИмя, 3) = "va_"
			ИЛИ СтрНайти(НРегИмя,"bdd_test") > 0
			ИЛИ СтрНайти(НРегИмя,"bdd test") > 0
			ИЛИ СтрНайти(НРегИмя,"bdd windows") > 0
			ИЛИ СтрНайти(НРегИмя,"bdd server") > 0
			ИЛИ СтрНайти(НРегИмя,"va_test") > 0
			Тогда
		...
		Для Каждого Файл Из Файлы Цикл
			...
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка результатов тестирования'"), 
				                         УровеньЖурналаРегистрации.Ошибка,,
				                         ,
				                         ИмяВСистемеСборки + Символы.ПС + ОписаниеОшибки());
				...
				Для Сч = 0 По МассивСтрок.Количество()-1 Цикл
					...
					Стр = СтрЗаменить(Стр, ",", "");
					Если Лев(Стр, 5) = """id"":" Тогда
						ДанныеДочернегоЗапуска.Вставить("id", Число(Сред(Стр, 6)));
					ИначеЕсли Лев(Стр, 13) = """project_id"":" Тогда
						ДанныеДочернегоЗапуска.Вставить("project_id", Число(Сред(Стр, 14)));
						Прервать;
					КонецЕсли;	
				КонецЦикла;	
...
Процедура СоздатьОшибкуЗапросаНаПомещение(Ветка)
	...
	Если ДанныеОткрытогоЗапросаНаСлияниеПоВетке.draft Тогда
		// Значит мердж реквест не готов к слиянию
		Возврат;
	КонецЕсли;	
